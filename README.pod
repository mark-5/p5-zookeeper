...
If set to false, all watchers will be triggered for session events, such as disconnecting to the ZooKeeper server. This means that watchers can be triggered multiple times, until the watcher event is triggered.
